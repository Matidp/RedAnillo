...
def gdelay_por_escenario(modo, caso, log=False, interp=0, title=""):
	...
	for i in range(len(esclist)):
		fx = vectores[modo][caso][esclist[i]]["nodo5"]["time_gdelay"]
		fy = vectores[modo][caso][esclist[i]]["nodo5"]["gdelay"]
		if interp != 0:
			linsp = np.linspace(1, 200, interp)
			yinterp = np.interp(linsp, fx, fy)
			axs.plot(linsp,yinterp, color=colors[i], label=titulos[esclist[i]])
		else:
			axs.plot(fx,fy, color=colors[i], label=titulos[esclist[i]])
	if log:
		plt.yscale('log')
...
def comp_delay_por_conexion(modo, caso, log=False, interp=0, title=""):
	...
	fig, axs = plt.subplots(1, len(esclist), figsize=(8*len(esclist),8)) #creo una figura y divido en pares para dibujar
	#plt.axis('equal')
	
	fig.suptitle("Retardo de cada Conexion. "+titulos[modo]+", "+titulos[caso], fontsize=24)

	maxv = 0
	for j in range(len(esclist)):
		for i in range(len(connections)):
			maxv = max(maxv, max(vectores[modo][caso][esclist[j]]["nodo5"][connections[i]]))

	for j in range(len(esclist)):
		axs[j].set_ylim(bottom=0, top=maxv)

		for i in range(len(connections)):
			fx = vectores[modo][caso][esclist[j]]["nodo5"][t_conns[i]]
			fy = vectores[modo][caso][esclist[j]]["nodo5"][connections[i]]
			if interp != 0:
				linsp = np.linspace(1, 200, interp)
				yinterp = np.interp(linsp, fx, fy)
		...
		axs[j].set_title(titulos[esclist[j]])
		plt.xlabel("Tiempo de simulacion (seg)")
		plt.ylabel("Retardo (segundos)")
...
def comp_esc_ocurs(mod, caso, metr):
	...
	fig, ax = plt.subplots(1,len(esclist), figsize=(6*len(esclist), 6))

	fig.suptitle(titulos[metr]+". "+titulos[mod]+", "+titulos[caso], fontsize=24)
	maxv = 0

	for j in range(len(esclist)):
		tar =  ocurrencias(mod,caso,esclist[j],"nodo5",metr)
		lists = sorted(tar.items()) # sorted by key, return a list of tuples
		x, y = zip(*lists) # unpack a list of pairs into two tuples
		maxv = max(maxv, max(y))
		df = pandas.DataFrame({
		...
		ax[j].set_title(titulos[esclist[j]])
...
def comp_ocupacion_buffers(model, caso):
	...
	if caso == "caso1":
		...
		esclist = escenarios2
	fig, ax = plt.subplots(1,len(esclist), figsize=(9*len(esclist), 7))

	fig.suptitle("Ocupación de los Buffers. "+titulos[model]+", "+titulos[caso], fontsize=24)

	vmax = 0

	for j,esc in enumerate(esclist):
